...
class _HouseholdFormScreenState extends State<HouseholdFormScreen> {
  ...
  void initState() {
    ...
    _headOfHouseholdController = TextEditingController(text: household?.headOfHousehold ?? '');
    _houseNumberController = TextEditingController(text: household?.houseNumber ?? '');
    _streetController = TextEditingController(text: household?.street ?? '');
    _neighborhoodController = TextEditingController(text: household?.neighborhood ?? '');
    _wardController = TextEditingController(text: household?.ward ?? '');
    _districtController = TextEditingController(text: household?.district ?? '');
    ...
    _populationController = TextEditingController(text: household?.population?.toString() ?? '');
    _notesController = TextEditingController(text: household?.notes ?? '');
    _longitudeController = TextEditingController(text: household?.longitude?.toString() ?? '');
    _latitudeController = TextEditingController(text: household?.latitude?.toString() ?? '');
    _loadDropdownData();
  }
  ...
  Future<void> _loadDropdownData() async {
    ...
    if (selectedCity.isNotEmpty) {
      final match = cities.firstWhere((c) => c['name'] == selectedCity, orElse: () => {});
      if (match.isNotEmpty) {
  ...
  Future<void> _onCityChanged(String name) async {
    ...
    final match = _cities.firstWhere((c) => c['name'] == name, orElse: () => {});
  ...
  @override
  void dispose() {
    _headOfHouseholdController.dispose(); _houseNumberController.dispose();
    _streetController.dispose(); _neighborhoodController.dispose();
    _wardController.dispose(); _districtController.dispose();
    _cityController.dispose(); _phoneController.dispose();
    _emailController.dispose(); _populationController.dispose();
    _notesController.dispose(); _longitudeController.dispose();
    _latitudeController.dispose(); super.dispose();
  }

  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) return;
    setState(() => _isSaving = true);
    String code = _isEditing ? widget.household!.householdCode : await _db.generateHouseholdCode();
    final h = Household(id: widget.household?.id, householdCode: code,
      headOfHousehold: _headOfHouseholdController.text.trim(),
      houseNumber: _houseNumberController.text.trim(), street: _streetController.text.trim(),
      neighborhood: _neighborhoodController.text.trim(), ward: _wardController.text.trim(),
      district: _districtController.text.trim(), city: _cityController.text.trim(),
    ...
    if (mounted) {
      ...
      if (ok) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_isEditing ? 'Updated' : 'Created'))); Navigator.pop(context); }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(_isEditing ? 'Edit Household' : 'Create Household')),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(16),
        child: Form(key: _formKey, child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _s('Basic Info'), const SizedBox(height: 8),
            TextFormField(controller: _headOfHouseholdController,
            ...
            Autocomplete<String>(
              ...
              optionsBuilder: (tv) {
                ...
                return tv.text.isEmpty ? names : names.where((e) => e.toLowerCase().contains(tv.text.toLowerCase()));
              },
              onSelected: (v) { _cityController.text = v; _onCityChanged(v); },
              fieldViewBuilder: (ctx, c, f, _) => TextFormField(
                controller: c, focusNode: f,
                decoration: const InputDecoration(labelText: 'City/Province', border: OutlineInputBorder(), suffixIcon: Icon(Icons.arrow_drop_down)),
              ),
            ),
            ...
            const SizedBox(height: 12),
            TextFormField(controller: _houseNumberController, decoration: const InputDecoration(labelText: 'House Number', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            TextFormField(controller: _streetController, decoration: const InputDecoration(labelText: 'Street', border: OutlineInputBorder())),
            const SizedBox(height: 24),
            _s('Contact'), const SizedBox(height: 8),
            ...
            TextFormField(controller: _populationController, decoration: const InputDecoration(labelText: 'Number of Members', border: OutlineInputBorder()), keyboardType: TextInputType.number),
